...
class Book():   
    ...
    def __repr__(self):
        return ("Book: {}\n"
                ...
                "\tIsbn10s              -> {}\n"
                "\tIsbn13s              -> {}\n"
                "\t\n").format(self.title, self.creator, self.contributors, self.publicationLocation,
                               self.publicationCountry, self.publisher, self.issuedYear, self.languages,
                               self.descriptions, self.subjects, self.types, self.identifiers, self.isbn10s,
                               self.isbn13s)
...
if __name__ == "__main__":
    ...
    for event, elem in ET.iterparse(FILENAME, events=events):
        if elem.tag == TAG_RDF_DESCRIPTION:
            description_counter += 1 if event == "start" else -1

            if description_counter == 0:
                books.append(BookBuilder.from_rdf_description(elem))
